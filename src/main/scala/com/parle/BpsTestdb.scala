...
object BpsTestdb {
   ...
   def main(args: Array[String]): Unit = {
				...
				for (table <- Source.fromFile(tablename).getLines) {
				  ...
				  var columnString =""	
				  val dropQuery = "drop table if exists testdb.dotnet_bps_"+table.toLowerCase()+"_temp"
					var createQuery = "create external table testdb.dotnet_bps_"+table.toLowerCase()+"_temp ( "
          ...
          import spark.sql
					...
					createQuery += columnString.substring(0,columnString.length()-1 ) + ") stored as TEXTFILE LOCATION '/user/darshana/'";
